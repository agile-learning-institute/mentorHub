...
The microservice principles used by, and deployed with DragonFly are aligned with the following influences:
...
1. Build, release run - Simple delivery pipeline consisting of only build-release-run
    - CI Pipelines should be very very simple and standardized by technology. Change management should be accomplished with tagging.
...
1. Port Binding
    - Simple network configurations using only 1 port in each service is an enforceable Dockerfile standard
1. Disposability
...
- Build Observable Systems, establish standards
  - Health and Config endpoints on all APIs to include version information, tags, and configuration sources
